...
def naturalMovement(preGame, actPos, x, y):
    global wins

    game = []
    for row in preGame:
        splitted = re.findall(r'.', ''.join(row))
        game.append(splitted)
    # print(game)
    ...
    if getRemain(game) == 1:
        finalPos = str(getFinalPos(game))
        if finalPos in wins.keys():
            wins[finalPos] += 1
        else:
            wins[finalPos] = 1
        return 0
...
def start():
    ...
    print('White space en ({}, {}) '.format(2, 1))
    # try:
